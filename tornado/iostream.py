...
#
# ************************************************************************
# socket 缓冲区满无法写时或者缓冲区空读不到数据时返回 EAGAIN， BSD 下使用 EWOULDBLOCK
# http://osr507doc.sco.com/cgi-bin/manform
#
# What is the Difference Between read() and recv() , and Between send() and write()?
#
# The only difference is that recv/send let you specify certain options for
# the actual operation . read/write are the 'universal' file descriptor
# functions while recv/send are slightly more specialized (for instance, you
# can set a flag to ignore SIGPIPE, or to send out-of-band messages...).
#
...
class BaseIOStream(object):
    ...
    def read_until_regex(self, regex, callback=None, max_bytes=None):
        ...
        从 stream 中读取数据直到遇上 regex 正则表达式匹配的字符串。连接关闭前（max_bytes=None）
        或者读取数据超过 ``max_bytes`` 指定字节大小的数据后，如果还没有匹配上将（关闭
        连接并）抛出 UnsatisfiableReadError。
        """
        # callback = None 时，future is None 使用 callback 回调，否则 future is not None
    ...
    def read_until(self, delimiter, callback=None, max_bytes=None):
        ...
        从 stream 中读取数据直到遇到 delimiter 指定的字符串。这实际上为读取 HTTP Header
        时遇到空行分隔符自动停止提供了方便。连接关闭前（max_bytes=None） 或者读取数据超过
        ``max_bytes`` 指定字节大小的数据后，如果还没有遇到 delimiter 将（关闭连接并）抛出
    ...
    def read_until_close(self, callback=None, streaming_callback=None):
        ...
        必须在 fd 关闭后调用，而 read_bytes() 方法中可能不调用。
        """
        future = self._set_read_callback(callback)
        self._streaming_callback = stack_context.wrap(streaming_callback)

        # 如果 socket 已经关闭那么就直接读取 buffer 中的数据并返回。
        # 与 read_bytes() 不同， callback 的执行条件是连接关闭，所以必须要调用。
        # self._try_inline_read() 现在的实现不会在检测到连接关闭时调用 callback，
        # 所以这里必须要检查一下 socket 的状态，以确保 callback 被调用。
        if self.closed():
    ...
    def _set_read_callback(self, callback):
        ...
        # 按照规范，在 callback 指定的情况下，self._read_future = None，
        ...
        if (self._read_bytes is not None and
            ...
            if self._read_buffer:
                while True:
                    ...
                    # 所以没有在每次循环后调用 _check_max_bytes。这样一来如果恶意发送大量数据而不发送分隔符的话，
                    # 除了造成内存的浪费，合并 chunk 的操作也会很昂贵，考虑到 fd 。
    ...
    def _handle_write(self):
        # 写数据直到发生 EWOULDBLOCK 异常或者 buffer 发送完毕。
        while self._write_buffer:
            try:
                ...
                # 上次 _merge_prefix 处理后的数据一个字节都还没发送，所以设置
                # self._write_buffer_frozen = True 避免下次 _handle_write() 操作时再调用
                # _merge_prefix  方法。
                if num_bytes == 0:
...
def _merge_prefix(deque, size):
    ...
    该方法提供了调整 ``deque`` 第 1 个 ``chunk`` 为指定 ``size`` 的功能，这样当读写
    buffer 时实际便可以直接操作 deque[0]，简化了对 buffer 的操作。
    """
